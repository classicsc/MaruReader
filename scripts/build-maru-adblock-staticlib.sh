...
if ! command -v rustup >/dev/null 2>&1; then
  echo "error: rustup is required to build MaruAdblockFFI for Apple targets" >&2
  exit 1
fi
...
for arch in "${resolved_archs[@]}"; do
  ...
  staticlib_path="$cargo_target_dir/$rust_target/$profile_dir/libmaru_adblock_ffi.a"
...
if [[ ${#staticlib_paths[@]} -eq 1 ]]; then
  cp "${staticlib_paths[0]}" "$output_dir/libmaru_adblock_ffi.a"
else
  xcrun lipo -create "${staticlib_paths[@]}" -output "$output_dir/libmaru_adblock_ffi.a"
fi
