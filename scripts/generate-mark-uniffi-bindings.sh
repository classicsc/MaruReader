...
CRATE_DIR="$ROOT_DIR/MaruMarkFFI"
OUTPUT_DIR="$ROOT_DIR/MaruMark/Generated/UniFFI"
...
library_path="$(find "$CRATE_DIR/target/debug" -maxdepth 1 -type f \( -name 'libmaru_mark_ffi.dylib' -o -name 'libmaru_mark_ffi.so' -o -name 'maru_mark_ffi.dll' \) | head -n 1)"
if [[ -z "$library_path" ]]; then
  echo "error: failed to locate host cdylib for MaruMarkFFI" >&2
  exit 1
fi
