...
struct MangaLibraryGrid: View {
    let model: MangaLibraryModel
    let coverImageLoader: MangaLibraryCoverImageLoader
    let onOpen: (NSManagedObjectID) -> Void
    let onEditMetadata: (NSManagedObjectID) -> Void
        ...
        LazyVGrid(
            ...
            ForEach(model.snapshots) { snapshot in
                if snapshot.isComplete {
                    ...
                    .contextMenu {
                        Button {
                            onEditMetadata(snapshot.objectID)
                        } label: {
                    ...
                    .confirmationDialog(
                        ...
                        Text(MangaLocalization.deleteConfirmationMessage(title: snapshot.title))
                    }
                } else {
                    MangaLibraryGridItem(
                        snapshot: snapshot,
    ...
    private func deleteConfirmationBinding(for objectID: NSManagedObjectID) -> Binding<Bool> {
        Binding(
            get: { model.pendingDeleteMangaID == objectID },
            set: { isPresented in
                if isPresented {
